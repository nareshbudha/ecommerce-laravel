        ...
        <div class="shopping-cart">
                        ...
                        <tr data-row-id="{{ $item->id }}">
                            ...
                            <td>
                                <div class="shopping-cart__product-item">
                                    <img loading="lazy" src="{{ asset('storage/' . $item->image) }}" width="120"
                            ...
                            <td>
                                <div class="qty-control position-relative">
                                    <form action="{{ route('cart.qty.decrease', ['rowId' => $item->id]) }}"
                                    ...
                                    <form action="{{ route('cart.qty.increase', ['rowId' => $item->id]) }}"
                                        method="POST" class="d-inline">
                                        @method('PUT')
                                        @csrf
                            ...
                            <td>
                                <span class="shopping-cart__subtotal">Rs{{ $item->price * $item->quantity }}</span>
                            </td>
                            <td>
                ...
                <div class="sticky-content">
                    <div class="shopping-cart__totals">
                        ...
                        <table class="cart-totals">
                            <tbody>
                                <tr>
                                    <th>Subtotal (<span id="selected-items-count">{{ $items->sum('quantity') }}</span>
                                        items)
                                    </th>
                                    <td id="selected-items-total">
                                        Rs{{ number_format($items->sum(fn($item) => $item->price * $item->quantity), 2)
                                        }}
                                    </td>
                                </tr>
                                <tr>
                    ...
                    <div class="mobile_fixed-btn_wrapper">
                        <div class="button-wrapper container">
                            <form id="checkout-form" action="{{ route('checkout') }}" method="POST">
                                @csrf
                                <button type="submit" class="btn btn-primary btn-checkout">PROCEED TO
                                    CHECKOUT</button>
                            </form>
                        </div>
                    </div>
            ...
            <div class="row">
                <div class="col-md text-center pt-5 bp-5">
                    ...
                    <a href="/shop/all" class="btn btn-info">Shop Now</a>
                </div>
            </div>
            @endif
        </div>
